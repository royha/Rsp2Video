...
namespace RSPro2Video
{
    public partial class RSPro2VideoForm : Form
    {
        ...
        // The list of ffmpeg tasks to run to create the video clips.
        List<FFmpegTask> FFmpegTasks = new List<FFmpegTask>();

        // The list of video clips to create to determine at .Add time if the clip is a duplicate clip.
        Dictionary<String,double> ClipsToCreate = new Dictionary<String,double>();

        // The list of video clips that will be duplicated by using a copy command, not generated by ffmpeg.
        Dictionary<String,List<String>> ClipsToDuplicate = new Dictionary<String, List<String>>();

        // The list of videos to output, and the clips needed to assemble them.
        List<VideoOutput> VideoOutputs = new List<VideoOutput>();

        // The index to VideoOutputs specifying which video clip list is being accessed.
        int VideoOutputIndex = 0;

        // Durations of each video clip.
        ConcurrentDictionary<String, Double> ClipDuration = new ConcurrentDictionary<String, Double>();

        // The list of clips that couldn't be created and must be removed from the VideoOutputs[VideoOutputIndex] list.
        ConcurrentBag<String> FailedClips = new ConcurrentBag<String>();
